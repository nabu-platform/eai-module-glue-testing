	...
	public static class GlueTestProjectRepository implements ScriptRepository {
		...
		public void refresh() throws IOException {
			...
			if (project.getConfiguration().getTests() != null) {
				for (GlueTestServiceArtifact testService : project.getConfiguration().getTests()) {
					scripts.put(ScriptUtils.getFullName(testService.getScript()), testService.getScript());
					if (provider == null) {
						provider = testService.getParserProvider();
					}
				}
			}
		}
